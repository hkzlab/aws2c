    ...
    int max_room_code)
{
    ...
    if(hardcoded_messages==false) {
        ...
        fprintf(of,TAB "printnewline();\n");
        fprintf(of, "}\n\n");
    } else if(output_messages_as_resource_file) {
        fprintf(of, "void show_messagenlf(const uint16_t id) FASTCALL\n"
                    "{\n"
                    TAB "boolean o;\n"
                    TAB "cpointer=0;\n"
                    TAB "bpointer=0;\n"
                    TAB "get_resource(id, resource_buffer);\n"
                    TAB "compressed=(char *)resource_buffer;\n"
                    TAB "do {\n"
                    TAB TAB "o=decode();\n"
                    TAB TAB "writesameln(decompress_b);\n"
                    TAB "} while(o);\n"
                    "}\n\n"
                    "void show_message(const uint16_t id) FASTCALL\n"
                    "{\n"
                    TAB "show_messagenlf(id);\n"
                    TAB "printnewline();\n"
                    "}\n");
    } else {
...
/** Create the code for the rooms' description in the output file.
    Room messages/descriptions.
    Return the total size occupied by room descriptions.
*/
unsigned int output_rooms_d(FILE *of, room* world, unsigned int rsize)
{
    unsigned int i,j;
    char *long_d;
    char *p;
...
/** Create the code for the rooms' description in the output file.
    ...
    Return the total size occupied by room descriptions.
*/
unsigned int output_rooms_s(FILE *of, room* world, unsigned int rsize)
{
    unsigned int i,j;
    char *long_d;
    char *p;
...
/** Create the code for the rooms' description in the output file.
    Return the total size occupied by room descriptions.
*/
unsigned int output_rooms_d_as_resources(FILE *of, room* world,
    unsigned int rsize)
{
    unsigned int i;
    unsigned int totalsize=0;
    ...
    for(i=0; i<rsize;++i) {
        fprintf(of, "long_d%d ",world[i].code);
        totalsize+=compress(of, encodechar(world[i].long_d),0);
        fprintf(of, "\n");
        fprintf(of, "short_d%d ",world[i].code);
        totalsize+=compress(of, encodechar(world[i].short_d),0);
        fprintf(of, "\n");
    }
...
/** Create the code for the header in the output file.
    Return the total size in byte occupied by the header.
*/
unsigned int output_game_info_as_resources(FILE *of, message* msg,
    unsigned int msize, info *header)
{
    unsigned int i,j;
    unsigned int size_d=0;
    unsigned int totalsize=0;
    ...
    fprintf(of, "areyousure ");
    totalsize+=compress(of, encodechar(AREYOUSURE),0);
    fprintf(of, "\n");
    fprintf(of, "exitrestart ");
    totalsize+=compress(of, encodechar(EXITRESTART),0);
    fprintf(of, "\n");
    if(!no_header) {
        buffer[0]='\0';
        strcat(buffer,header->name);
        ...
        strcat(buffer,awsversion);
        fprintf(of, "header ");
        totalsize+=compress(of, encodechar(buffer),0);
        fprintf(of, "\n");
        if(strcmp(header->description,"")!=0) {
            fprintf(of, "headerdescription ");
            totalsize+=compress(of, encodechar(header->description),0);
            fprintf(of, "\n");
        } else {
            no_header_description=true;
        }
...
/** Create the code for the objects in the output file. Descriptions. */
void output_objects_d(FILE *of, object* obj, unsigned int osize)
{
    unsigned int i;
    if(compress_messages==true) {
        for(i=0; i<osize;++i) {
...
/** Create the code for the objects in the output file. Descriptions. */
void output_objects_d_as_resources(FILE *of, object* obj, unsigned int osize)
{
    unsigned int i;
    if(compress_messages==false || hardcoded_messages==false) {
        fprintf(stderr,"The -R option requires -m and -c.\n");
    ...
    for(i=0; i<osize;++i) {
        ...
        compress(of, encodechar(obj[i].desc),0);
        fprintf(of, "\n");
...
/** Create the code for the objects in the output file. Structure.*/
void output_objects_s(FILE *of, object* obj, unsigned int osize)
{
